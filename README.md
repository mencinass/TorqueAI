...
O estado operacional atual, incluindo Podman, Ollama, ingestão automática, backups e troubleshooting, está documentado em [docs/PROJECT_STATUS.md](docs/PROJECT_STATUS.md).
...
1. **100% Pure Python**: Todo o backend, conexões, ferramentas e agentes rodam exclusivamente em Python puro (sem compiladores C/C++, gcc ou bibliotecas nativas compiladas).
...
O ambiente foi configurado para ser 100% compatível com **Linux (Pop!_OS / Ubuntu)**, **macOS** e **Windows**, suportando nativamente tanto **Podman** quanto **Docker**.
...
No Windows com **Podman** ou **Docker**, utilize o script [`run.ps1`](file:///c:/Users/mathe/Documents/PROJETO/helpMec/run.ps1):
...
Instale o Ollama, inicie o serviço e baixe um modelo de chat:
...
Ative a geração local no ambiente do backend:
...
Com a API em execução, abra `http://localhost:8000/api/v1/chat/`. O modelo recebe somente os trechos recuperados dos manuais; sem evidência suficiente, o chat não gera uma conclusão técnica.
...
As especificações ativas e validadas residem no diretório [`openspec/specs/`](file:///c:/Users/mathe/Documents/PROJETO/helpMec/openspec/specs/).
